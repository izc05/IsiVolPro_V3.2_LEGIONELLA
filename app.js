...
function startRecirFromQueue(idx) {
  ...
  $("tankWater").style.height = "0%";
  $("tankPct").textContent = "0%";
  $("timerMeta").textContent = `Progreso: 0% · Transcurrido 00:00 · Restante ${fmtTime(recirTimer.durationMs)}`;
...
$("btnAddPunto")?.addEventListener("click", async () => {
  ...
  toast(`OT ${code} añadida a pendientes ✅${recirTimer.running ? " · cronómetro activo" : ""}`, "ok", "Recirculación");
  soundSave();
  $("puntoCode").value = "";
...
$("btnStartTimer")?.addEventListener("click", () => {
  if (recirTimer.running) return toast("Ya hay un cronómetro en marcha.", "warn");
  if (!recirQueue.length) return toast("Añade OT pendientes con el botón '✅ Añadir'.", "info");
...
function recirTimerTick() {
  ...
  $("timerMeta").textContent = `Progreso: ${Math.round(pct*100)}% · Transcurrido ${fmtTime(elapsed)} · Restante ${fmtTime(left)}`;
